"""TESR YOLO Trainer - Step 5: Export best.pt for Edge devices.

Pick your target device - the right format is chosen for you:

    python export.py --target pi        # Raspberry Pi  -> NCNN (fastest on Pi CPU)
    python export.py --target jetson    # Jetson        -> TensorRT engine (FP16)
    python export.py --target onnx      # anything else -> ONNX (portable)
    python export.py --sample-only      # no export - just write the sample code for best.pt
...
WEIGHTS = \"__WEIGHTS__\"   # written by export.py - change to any other model file
...
This file was generated by `export.py` and is wired to the model: `__WEIGHTS__`
...
ไฟล์นี้ถูกสร้างโดย `export.py` และผูกกับโมเดล: `__WEIGHTS__`
...
def main():
    ...
    print("    python run.py --weights %s" % out)
    print("    python run.py --weights %s --headless   # over SSH, no display" % out)
    print("    python sample_predict.py                  # minimal integration demo")
    if args.target == "pi" and not args.imgsz:
        print("Too slow on the Pi? Re-export with:  python export.py --target pi --imgsz 320")
