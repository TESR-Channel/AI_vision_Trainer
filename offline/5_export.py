...
Every export also writes SAMPLE CODE next to your model:
    ...
    SAMPLE_README.md    how to use it in your own project + MQTT/Node-RED example
...
SAMPLE_README = '''# ใช้โมเดลของคุณในโค้ดตัวเอง — sample_predict.py
...
cli = mqtt.Client()
cli.connect("localhost", 1883)            # เปลี่ยนเป็น IP ของ MQTT broker
...
ฝั่ง Node-RED: ใส่ node **mqtt in** subscribe topic `tesr/vision` + **json** node
ก็ได้ข้อความพร้อมใช้ต่อใน flow ทันที
...
def write_samples(weights_path):
    ...
    print("    SAMPLE_README.md    (use in your own code + MQTT/Node-RED example)")
